...
import { Camera, Plus, Trash2, Save, RotateCcw, Shirt, CheckCircle2, CloudSun, Download, Sparkles, Loader2, X, User, Wand2, Ruler, ThermometerSun, Pencil, LayoutGrid, Home, Menu } from 'lucide-react';
...
const App = () => {
  ...
  // Mobile Tab State (New for Mobile UI)
  const [mobileView, setMobileView] = useState<'fitting' | 'wardrobe'>('fitting');

  // Body Model State
  const [bodyImage, setBodyImage] = useState<string | null>(null);
  
  // Body Stats State - Initialized with User's Data
  ...
  const handleWeatherRecommendation = async () => {
    if (wardrobe.length === 0) {
        alert("衣櫃是空的，請先新增衣物！");
        setMobileView('wardrobe'); // Switch to wardrobe on mobile to hint adding
        return;
    }
    setIsThinking(true);
    setWeatherAdvice(null);
    setSelectedItems([]);
    setMobileView('fitting'); // Ensure we are on fitting view to see results
         ...
         const loadAndDraw = async (item: ClothingItem, idx: number) => {
            return new Promise<void>((r) => {
                ...
                img.onload = () => {
                    ...
                    // Simple Grid logic for right side
                    if (count === 1) {
